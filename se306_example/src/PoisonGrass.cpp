...
PoisonGrass::PoisonGrass(std::string robot_name, int argc, char **argv,double px,double py, std::string robot_number):Robot(robot_name,argc,argv,px,py,robot_number) {
  moistCont = 0;
  maxMoistCont = 0;
  height = 5;
  maxHeight = 20;
...
void PoisonGrass::eatenCallback(const std_msgs::String::ConstPtr& msg) {
  ...
  if (this->height < 5) {
    message.data = robot_name+robot_number+": Poison";
...
ros::NodeHandle PoisonGrass::run(){
  ...
  ros::Subscriber eatSub = n.subscribe<std_msgs::String>(robot_name+robot_number+"/eat",1000, &PoisonGrass::eatenCallback,this);
...
{
  PoisonGrass robot = PoisonGrass("PoisonGrass",argc,argv,10,20,"One");
  robot.run();
  return 0;
}
